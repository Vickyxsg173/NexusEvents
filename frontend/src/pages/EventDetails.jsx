        ...
        <div className="absolute bottom-0 left-0 w-full">
          <div className="max-w-5xl mx-auto px-6 pb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex-1">
              ...
              <p className="text-lg text-gray-300 font-medium flex items-center">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {event.tags && event.tags.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-8">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Relevant Tags</h3>
              <div className="flex flex-wrap gap-3">
                {event.tags.map((tag, idx) => (
                  <span key={idx} className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6 sticky top-24">
            
            <div>
              <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Schedule</h3>
              <div className="flex items-start">
                <Calendar className="w-5 h-5 text-brand-500 mr-3 mt-0.5" />
                <div>
                  <p className="text-gray-900 dark:text-white font-medium">{formatDate(event.start_date)}</p>
                  {event.end_date && (
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">To: {formatDate(event.end_date)}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Location</h3>
              <div className="flex items-start">
                <MapPin className="w-5 h-5 text-brand-500 mr-3 mt-0.5" />
                <div>
                  <p className="text-gray-900 dark:text-white font-medium">{event.mode}</p>
                  {event.venue && event.venue !== 'Online' && (
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{event.venue}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {(event.prize_pool || event.eligibility) && (
              <>
                ...
                {event.prize_pool && (
                  <div>
                    <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Prize Pool</h3>
                    <div className="flex items-start">
                      <Trophy className="w-5 h-5 text-amber-500 mr-3 mt-0.5" />
                      <p className="text-gray-900 dark:text-white font-medium">{event.prize_pool}</p>
                    </div>
                  </div>
                )}
                ...
                {event.eligibility && (
                  <div className="pt-2">
                    <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Eligibility</h3>
                    <div className="flex items-start">
                      <Users className="w-5 h-5 text-blue-500 mr-3 mt-0.5" />
                      <p className="text-gray-900 dark:text-white font-medium">{event.eligibility}</p>
                    </div>
                  </div>
                )}
              </>
